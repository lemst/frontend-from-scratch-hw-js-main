...
const displayedImage = document.querySelector('#web-tech-image');
...
function getIndexOfDisplayedImage() {
  let srcAttribute = displayedImage.getAttribute('src');
  let indexOfDisplayedImage = WEB_TECH_IMAGES.indexOf(srcAttribute);
  return indexOfDisplayedImage;
}

prev.addEventListener('click', function () {
  i = getIndexOfDisplayedImage();

  if (i === 0) {
    displayedImage.setAttribute('src', WEB_TECH_IMAGES[WEB_TECH_IMAGES.length - 1]);
  } else {
    displayedImage.setAttribute('src', WEB_TECH_IMAGES[i - 1]);
  }
})
...
next.addEventListener('click', function () {
  ...
  if (i === WEB_TECH_IMAGES.length - 1) {
    displayedImage.setAttribute('src', WEB_TECH_IMAGES[0]);
  } else {
    displayedImage.setAttribute('src', WEB_TECH_IMAGES[i + 1]);
  }
})